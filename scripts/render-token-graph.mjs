...
export function renderSnapshot(snapshot) {
  ...
  for (let value = 0; value <= ceiling + step / 2; value += step) {
    ...
    horizontalGrid.push(
      `<path d="M${plot.x} ${pointY.toFixed(2)}H${plot.x + plot.width}" stroke="${FRAME}" stroke-width="1"/>`,
      `<text x="${plot.x - 18}" y="${(pointY + 4).toFixed(2)}" text-anchor="end">${escapeXml(compactNumber(value))}</text>`
    );
  }

  const verticalGrid = [0, 7, 14, 21, 29]
    .map((index) => `<path d="M${x(index).toFixed(2)} ${plot.y}V${plot.y + plot.height}" stroke="${GRID}" stroke-width="1"/>`)
    .join('');
  ...
  <defs>
    ...
    <linearGradient id="area" x1="0" y1="0" x2="0" y2="1">
      ...
      <stop offset="1" stop-color="${CYAN}" stop-opacity=".01"/>
    </linearGradient>
${wordmarkDef([152, 70, 232, 100])}
${chromeDefs()}
  </defs>
...
${chromeHeader({
    eyebrow: 'PERSONAL INSTRUMENT / 30D',
    word: 'token',
    accent: 'use',
    subtitle: 'CUMULATIVE / ALL SURFACES',
    readout: totalLabel,
    asOfDate
  })}
  ...
  <path d="M${endX.toFixed(2)} ${plot.y}V${plot.y + plot.height}" stroke="${VIOLET}" stroke-width="1" stroke-dasharray="2 6" opacity=".55"/>
  <circle cx="${endX.toFixed(2)}" cy="${endY.toFixed(2)}" r="6" fill="${VIOLET}" fill-opacity=".18"/>
  <circle cx="${endX.toFixed(2)}" cy="${endY.toFixed(2)}" r="3.5" fill="${INK}" stroke="${VIOLET}" stroke-width="2"/>

  <g fill="${MUTED}" font-family="${MONO}" font-size="9" letter-spacing="1.5">
...
export function renderGraphBlock(asOfDate) {
  ...
  return `${GRAPH_START}
<p align="center">
  <img src="https://raw.githubusercontent.com/mystxcal/mystxcal/main/assets/token-use.svg?v=${version}-${CACHE_REVISION}" alt="Cumulative token use over the past 30 days" width="100%">
</p>
${GRAPH_END}`;
}
