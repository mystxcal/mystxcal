...
test('renders well-formed svg carrying the real colours', () => {
  ...
  const widths = [...svg.matchAll(/<rect x="([\d.]+)" y="148" width="([\d.]+)"/g)];
  assert.ok(widths.length > 0);
  for (const [, x, w] of widths) assert.ok(Number(x) + Number(w) <= 1138.5);
});

test('the quarter scale spans the bar exactly', () => {
  const svg = renderCard(summarise(repositories), '2026-08-21');
  assert.match(svg, /<text x="62.00" y="189" text-anchor="start">0<\/text>/);
  assert.match(svg, /<text x="1138.00" y="189" text-anchor="end">100%<\/text>/);
  assert.match(svg, /<text x="600.00" y="189" text-anchor="middle">50<\/text>/);
});
